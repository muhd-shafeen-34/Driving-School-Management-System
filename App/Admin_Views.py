...
def add_staff_save(req):
    if req.method != 'POST':
        ...
        license = req.FILES.get('license')
        try:
            user = models.CustomUser.objects.create_user(email=email,password=raw_password,role="instructor")
            Instructor = models.Instructor.objects.create(user = user, name = fullname,age = age, specialization = specialization, phone = phone, profile_picture = profile_pic, license = license)
            user.save()
            Instructor.save()
            print("File path:", Instructor.profile_picture.path)
